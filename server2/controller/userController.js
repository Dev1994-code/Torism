...
exports.register = asyncMiddleware(async (req, res) => {
  ...
  const usersByUsername = await User.findOne({ username: username });
  if (usersByUsername) {
    return res
      .status(409)
  ...
  // Check if a user with the same contact information already exists
  const usersByContact = await User.findOne({
    contactInformation: contactInformation,
  });
  if (usersByContact) {
    return res
      .status(408)
      .json({ error: "User with this contact information already exists." });
  }
